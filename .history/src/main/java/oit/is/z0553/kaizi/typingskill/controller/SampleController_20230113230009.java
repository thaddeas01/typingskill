...
public class SampleController {
  ...
  public String hantei(ModelMap model, @RequestParam String prob, @RequestParam int miss, @RequestParam int score,
    ...
    if (prob.equals(answer)) {
      point++;
      String spell3 = vocabmapper.selectRadId(rad.nextInt(
        maxdata));
      if(score>15){
        for(int j=0;j<5;j++){
          for(int i=0; i<spell3.length();i++){
            if(boolean rand_bool1 = ThreadLocalRandom.current().nextBoolean();)
          }
      }
    } else {
      mistake++;
    ...
    String spell0 = vocabmapper.selectRadId(rad.nextInt(
        maxdata));
    model.addAttribute("spell", spell0);
    model.addAttribute("miss", mistake);
    model.addAttribute("score", point);
    return ("single.html");
  }
